...
public class DataLogger     // A function that runs the data logging data
{
    ...
    public void LogData()       // This is what is run when logdata is called.
    {
        if (isExternalStorageWritable())        // Checks if the storage is writable
        {
            ...
            {
                File main_directory = new File(Directory);    // Path to file in the storage of the device

                if (!main_directory.isDirectory())    // If there is no directory with that name
                {
                    Log.i("Data Logger", "Making a directory called " + main_directory);     // Logs on Console.
                    main_directory.mkdirs();        // Make a directory with the name.
                }
    ...
    public void WriteData()     // This writes the data to the sdcard.
    {
        if (isExternalStorageWritable())        // Checks if we can write data to the card.
        {
            try     // Tries to do this
            {
                ...
                if (!main_directory.isDirectory())        // If there is no directory with the name
                {
                    Log.i("Data Logger", "Making a directory called " + main_directory);     // Logs on Console.
                    main_directory.mkdirs();        // Do nothing.
                }
